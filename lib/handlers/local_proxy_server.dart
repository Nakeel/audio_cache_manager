...
class LocalProxyServer {
  ...
  Future<void> start() async {
    ...
    // _router.get('/audio/<trackId>', (Request request, String trackId) async {
    //   AppLogger.info('Request for HLS manifest: $trackId, headers: ${request.headers}', name: 'LocalProxyServer');
    //   final CacheEntry? entry = await metadataStore.get(trackId);
    //   if (entry == null || !entry.isHls || entry.hlsManifestFilePath == null) {
    //     AppLogger.error('Track $trackId not found or not HLS', name: 'LocalProxyServer');
    //     return Response.notFound('Track not found or not HLS');
    //   }
    //
    //   final File manifestFile = File(entry.hlsManifestFilePath!);
    //   if (!await manifestFile.exists()) {
    //     AppLogger.error('HLS master manifest not found: ${entry.hlsManifestFilePath}', name: 'LocalProxyServer');
    //     return Response.internalServerError(body: 'HLS manifest not found.');
    //   }
    //
    //   String manifestContent = await manifestFile.readAsString();
    //   manifestContent = _rewriteHlsManifest(manifestContent, trackId, port, proxySegmentRoute: '/hls_segments');
    //   AppLogger.info('Serving HLS master manifest for $trackId, content length: ${manifestContent.length}', name: 'LocalProxyServer');
    //
    //   return Response.ok(manifestContent, headers: {
    ...
    _router.get('/audio/<trackId>', (Request request, String trackId) async {
      ...
      if (!await hlsDir.exists()) {
        AppLogger.error('HLS cache directory missing: ${entry.hlsLocalPath}', name: 'LocalProxyServer');
        return Response.notFound('HLS cache directory not found');
      }
      String manifestContent = await manifestFile.readAsString();
      manifestContent = _rewriteHlsManifest(manifestContent, trackId, port, proxySegmentRoute: '/hls_segments');
      return Response.ok(manifestContent, headers: {
    ...
    _router.get('/hls_segments/<trackId>/<path|.*>', (Request request, String trackId, String path) async {
      AppLogger.info('HLS segment request: track $trackId, path: $path, headers: ${request.headers}', name: 'LocalProxyServer');
      final CacheEntry? entry = await metadataStore.get(trackId);
      if (entry == null || !entry.isHls || entry.hlsLocalPath == null) {
        AppLogger.error('HLS track $trackId not found or not HLS', name: 'LocalProxyServer');
      ...
      if (!await hlsFile.exists()) {
        ...
        return Response.notFound('HLS segment or manifest not found.');
      }

      String contentType = path.endsWith('.m3u8') ? 'application/x-mpegURL' : 'video/mp2t';
      Uint8List fileBytes;
      final int fileLength = await hlsFile.length();
      String rangeHeader = request.headers['range'] ?? '';

      if (rangeHeader.isNotEmpty && !path.endsWith('.m3u8')) {
        final rangeMatch = RegExp(r'bytes=(\d+)-(\d+)?').firstMatch(rangeHeader);
        if (rangeMatch != null) {
          final start = int.parse(rangeMatch.group(1)!);
          final end = rangeMatch.group(2) != null ? int.parse(rangeMatch.group(2)!) : fileLength - 1;
          if (start >= fileLength || end >= fileLength) {
            AppLogger.error('Invalid range request for $fullLocalPath: $rangeHeader', name: 'LocalProxyServer');
            return Response(416, body: 'Range not satisfiable');
          }
          fileBytes = await hlsFile.readAsBytes().then((bytes) => bytes.sublist(start, end + 1));
          if (entry.isEncrypted && path.endsWith('.ts')) {
            try {
              fileBytes = AESHelper.decrypt(fileBytes);
            } catch (e, st) {
              AppLogger.error('Error decrypting HLS segment $path for $trackId: $e', error: e, stackTrace: st, name: 'LocalProxyServer');
              return Response.internalServerError(body: 'Error decrypting HLS segment: $e');
            }
          }
          AppLogger.info('Serving partial HLS segment: $path, range: bytes $start-$end/$fileLength', name: 'LocalProxyServer');
          return Response.ok(206,  headers: {
            'Content-Type': contentType,
            'Content-Length': fileBytes.length.toString(),
      ...
      fileBytes = await hlsFile.readAsBytes();
      if (entry.isEncrypted && path.endsWith('.ts')) {
        try {
          fileBytes = AESHelper.decrypt(fileBytes);
        } catch (e, st) {
      ...
      AppLogger.info('Serving HLS file: $path for $trackId, size: ${fileBytes.length}', name: 'LocalProxyServer');
      return Response.ok(fileBytes, headers: {
        'Content-Type': contentType,
        'Content-Length': fileBytes.length.toString(),
        'Accept-Ranges': 'bytes',
  ...
  //   final rewrittenContent = manifestContent.replaceAllMapped(urlPattern, (match) {
  //     String originalPath = match.group(1)!;
  //     final String resolvedPath = p.join(p.dirname(basePath), originalPath);
  //     final String fullProxyPath = 'http://127.0.0.1:$port$proxySegmentRoute/$trackId/$resolvedPath';
